//rewrite where valid is set to 0 and then if it can move, set it to 1
#include<iostream>
#include<vector>
		...
		if (turn == 2)
		{
			for (int i = 0; i < 8; i++)
			{
				for (int j = 0; j < 8; j++)
				{
					...
					if (board[i][j] == blackPiece)
					{
						std::cout << j << ", " << i << " = " << drawCount << std::endl;
					}
					*/
				}
			}
...
int main()
{
	...
	//test all tie game scenarios
	
	board[1][0] = whiteKing;
	board[3][0] = whiteKing;
	board[0][3] = whitePiece;
	board[3][2] = whitePiece;
	board[1][2] = whitePiece;
	board[4][3] = whitePiece;
